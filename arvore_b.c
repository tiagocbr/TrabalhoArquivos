...
ARVORE_B* arvore_criar(char* indices){
    ...
    raiz.altura_no = 0;
    raiz.n_chaves = 0;
    raiz.RRN_filhos = {-1,-1,-1,-1};
    raiz.elementos = {NULL,NULL,NULL};
...
ll arvore_buscar(ARVORE_B* arvore,int chave){
    ...
    return arvore_buscar_recursivo(arvore->RRN_raiz,chave,arquivo);
    fclose(arquivo);
}

elem inserir_elemento_em_no(NO no, elem new_elem, RRN_NO, ARVORE_B* arvore,FILE* indices){ //insere o elemento e retorna o elemento que vai subir do split, caso haja
        elem* elementos = no.elementos;
        int* RRN_filhos = no.RRN_filhos;
        ...
        if(no.n_chaves < 3){ //insere nesse NO o novo elemento, sem split
            ...
            int aux=0;
            for(int j=0;j<4;j++){
                if( (elementos[j].chave>chave) && aux==0 ){
    ...
    for(int i=0;i<no.n_chaves;i++){
        if(elementos[i].chave>chave || i == no.n_chaves - 1){
            ...
            if(RRN_filho==-1){ //A inserção vai ocorrer agora, pois chegamos em uma folha
                ...
                elem elemento_a_ser_inserido = arvore_inserir_recursivo(RRN_filho,chave,indices);
                if(elemento_a_ser_inserido == NULL) return NULL;

                //inserir o elemento_a_ser_inserido na posicao posicao_inserida e retornar o elemento que subirá do split,caso haja
...
bool arvore_inserir(ARVORE_B* arvore,int chave,ll offset){
    FILE* arquivo = fopen(arvore->indices,"rb+");
    return arvore_inserir_recursivo(arvore->RRN_raiz,chave, offset,arquivo, arvore);
    fclose(arquivo);
}
